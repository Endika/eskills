...
check: ## Validate frontmatter, JSON, and design caps (run before pushing)
	...
	[ "$$skills" -le 15 ] || { echo "✗ >15 skills ($$skills) — cap (raised 10→11 for context-budget, 11→12 for exploit-hunt, 12→13 for stacks — which houses ALL per-stack guides under references/, so new stacks never add a skill — 13→14 for gdpr, 14→15 for comms — outward text to a human; repo text stays in standards)"; fail=1; }
